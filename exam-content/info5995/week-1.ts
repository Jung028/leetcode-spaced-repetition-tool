...
const PAPER: ExamPaperSeed = 
  {
    ...
    questions: [
      ...
      },
      {
        ...
        modelAnswer: "Build securely (understand assets → identify threats → test for weaknesses) → Watch and protect (monitor → detect suspicious activity → patch/improve) → Respond (contain → recover → investigate) → Learn and strengthen (fix weaknesses, feed lessons into the next cycle). It's a continuous loop, not a straight line.",
      },
      {
        type: "short",
      ...
      },
      {
        ...
        modelAnswer: "Goal: obtain the student's university login credentials (to access grades, financial info, or pivot to other accounts). Capability: can craft a convincing fake email/login page mimicking the university portal, can spoof sender addresses. Attack vector: email lands in inbox → student clicks link → enters credentials on fake page → attacker captures and reuses them, exploiting the student's trust in familiar branding and urgency cues rather than a technical software flaw.",
      },
      {
        type: "short",
      ...
      },
      {
        ...
        modelAnswer: "Build securely → Watch and protect → Respond → Learn and strengthen. Before deployment, security work is preventive and design-time (threat modeling, secure coding, manual audit, defining what needs protecting); after deployment, it shifts to operational and reactive activities against a live system (monitoring, intrusion detection, patching newly found issues) — the system is now exposed to real traffic and real attackers, so the goal moves from \"design this well\" to \"watch this constantly.\"",
      },
      {
        type: "short",
        ...
        modelAnswer: "A password manager's trust assumption might be \"the master password is never intercepted by anything running on the user's device.\" If a keylogger captures that one password, every credential the manager stores is instantly compromised regardless of how strong the encryption or how many other controls exist — it is the shortest board because every other layer of protection is downstream of, and depends on, that single assumption holding.",
      },
      {
        type: "scenario",
      ...
      },
      {
        ...
        modelAnswer: "a) New trust assumption: \"every firmware update pushed through this channel genuinely comes from the vendor and has not been tampered with in transit or at the update server.\" b) Integrity attack: an attacker compromises the vendor's update server (or intercepts the update channel) and pushes malicious firmware to thousands of doorbells at once, silently adding a backdoor that streams video to the attacker. Lifecycle control: before deployment, require all firmware updates to be cryptographically signed and verified on-device before installation, so a device rejects any update that isn't signed by the vendor's private key — this closes the exact trust assumption identified in (a) instead of just hoping the update channel is never compromised.",
      },
    ],
  }
