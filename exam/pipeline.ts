...
const SELF_CONTAINED_RULE =
  `SELF-CONTAINED QUESTIONS (hard rule): every question must be self-contained — answerable from the question text itself, any reading card in the paper, and the student's knowledge. A question must never say "the deck", "the slides", or "the lecture" says / matches / shows something unless it either states the actual numbers or facts in the question itself, or points the student to a reading card that contains them. The student cannot see the deck or slides.`;
...
• every question is self-contained: treat any question that says "the deck", "the slides", or "the lecture" says / matches / shows something, without stating the actual numbers or facts itself or pointing to a reading card that contains them, as a failure, and fix it (rewrite the question to state the facts, or add them to a reading card);
