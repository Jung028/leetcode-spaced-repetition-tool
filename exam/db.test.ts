...
test("seeds every paper for the course, unsubmitted", () => {
  ...
  expect(rows.length).toBe(10); // INFO5995 Week 1-4 papers, Week 5 and Week 6 two papers each, plus Week 7 two papers
  expect(rows.every((r) => r.submitted_at === null)).toBe(true);
  expect(rows.map((r) => r.week).sort()).toEqual([1, 2, 3, 4, 5, 5, 6, 6, 7, 7]);
  expect(rows.map((r) => r.paper_number).sort()).toEqual([1, 1, 1, 1, 1, 1, 1, 2, 2, 2]);
});
...
test("two different courses' rows are independent — course scoping partitions correctly", () => {
  ...
  expect(listExamPaperRows(db, COURSE).length).toBe(10); // unaffected by OTHERCOURSE's row — INFO5995's own Week 1-4 papers, Week 5's and Week 6's two papers each, plus Week 7 two papers
});
...
test("migrateExam upgrades a pre-existing paper_day-keyed db, recovering (week, paperNumber) by content position", () => {
  ...
  expect(listExamPaperRows(legacyDb, "INFO5995").length).toBe(10);
