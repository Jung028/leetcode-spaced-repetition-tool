...
test("seeds every paper for the course, unsubmitted", () => {
  ...
  expect(rows.length).toBe(11); // INFO5995 Week 1-4 papers, Week 5 and Week 6 two papers each, plus Week 7 two papers, plus Week 8 one paper
  expect(rows.every((r) => r.submitted_at === null)).toBe(true);
  expect(rows.map((r) => r.week).sort()).toEqual([1, 2, 3, 4, 5, 5, 6, 6, 7, 7, 8]);
  expect(rows.map((r) => r.paper_number).sort()).toEqual([1, 1, 1, 1, 1, 1, 1, 1, 2, 2, 2]);
});
...
test("two different courses' rows are independent — course scoping partitions correctly", () => {
  ...
  expect(listExamPaperRows(db, COURSE).length).toBe(11); // unaffected by OTHERCOURSE's row — INFO5995's own Week 1-4 papers, Week 5's and Week 6's two papers each, plus Week 7 two papers, plus Week 8 one paper
});
...
test("migrateExam upgrades a pre-existing paper_day-keyed db, recovering (week, paperNumber) by content position", () => {
  ...
  // fresh-seeds INFO5995's Week 2, Week 3, Week 4, Week 5 (discussion + lecture), Week 6 (tutorial + lecture) and Week 7 (two lecture papers) papers (not present in the legacy fixture).
  expect(listExamPaperRows(legacyDb, "INFO5995").length).toBe(11);
...
test("a migration failure rolls back cleanly, leaving the original paper_day-shaped tables intact", () => {
  ...
  // The migrated row plus INFO5995's Week 2, Week 3, Week 4, Week 5 (discussion + lecture), Week 6 (tutorial + lecture) and Week 7 (two lecture papers) papers, fresh-seeded by seedNewPapers.
  expect(listExamPaperRows(legacyDb, "INFO5995").length).toBe(11);
});
