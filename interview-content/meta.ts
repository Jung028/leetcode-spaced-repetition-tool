...
export const META: CompanySystemDesignSeed = {
  ...
  Client["Client apps"]
  API["Link API - stateless, many hosts"]
  Coord["ID range / Snowflake service"]
  KV[("short_code to long_url store - sharded by hash")]
  Edge["CDN / edge cache"]
  Mem["In-memory cache - Redis, long TTL"]

  Client -->|"POST /links"| API
  API -->|"claim counter range"| Coord
  API -->|"base62 code, write mapping"| KV
  Client -->|"GET short code"| Edge
  Edge -->|"hit: 301 redirect"| Client
  Edge -->|"miss"| Mem
  Mem -->|"miss"| KV
  KV -->|"long_url"| Mem
  Mem --> Edge`,
  ...
  Author["User publishes post"]
  PostSvc["Post storage service - sharded by post ID"]
  Threshold{"Follower count above threshold?"}
  Push["Fan-out on write - push post ID into each follower feed list"]
  Skip["Skip fan-out - merged at read time"]
  FeedStore[("Per-user feed lists - bounded post IDs, fast KV store")]
  Reader["Viewer opens feed"]
  FeedAPI["Feed API"]
  Ranker["Ranking service - recency, viewer affinity, content type"]

  Author --> PostSvc --> Threshold
  Threshold -->|"ordinary user"| Push
  Threshold -->|"celebrity"| Skip
  Push --> FeedStore
  Reader --> FeedAPI
  FeedAPI --> FeedStore
  FeedAPI -->|"merge celebrity posts"| Skip
  FeedAPI --> Ranker
  Ranker -->|"hydrate content"| PostSvc
  Ranker -->|"ranked page + cursor"| Reader`,
    },
  ],
};
